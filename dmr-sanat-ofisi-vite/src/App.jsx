      ...
      <section id="works" className="relative">
        {scenes.map((scene) => (
          <motion.article key={scene.title} initial={{ opacity: 0, y: 70 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.24 }} transition={{ duration: 1, ease: "easeOut" }} className="relative min-h-screen overflow-hidden border-t border-white/10">
            ...
            <button onClick={() => { setActiveGallery(scene); setActiveImage(0); }} className="absolute right-5 top-28 z-20 flex items-center gap-3 rounded-full border border-[#D8B16F]/70 bg-black/55 px-5 py-3 text-[#F1D091] shadow-[0_20px_80px_rgba(0,0,0,0.65)] backdrop-blur-xl transition hover:scale-105 hover:bg-[#D8B16F] hover:text-black md:right-10 md:top-32"
